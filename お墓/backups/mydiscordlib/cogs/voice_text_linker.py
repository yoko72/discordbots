...
class VoiceTextLinker(Cog):
    ...
    def __init__(self, bot: discord.ext.commands.Bot,
                 db: Dict, setting_json_path: str = None, saves_settings=True) -> None:
        """
        Parameters
        ----------
        db : Dict[int:int]
    ...
    def make_sure_file_exists(self):
        if not self.saves_settings:
            return
        if not self.setting_json_path.exists():
            self.saves_settings()

    def load_settings(self):
        self.db.update(json.load(self.setting_json_path))
