...
from .utils.timers import AioDeltaCountdown
...
class EmojiTimer(Cog):
    ...
    async def on_ready(self) -> None:
        self.build_emoji_dicts()

    def build_emoji_dicts(self) -> None:
    ...
    async def countdown(self, ctx: discord.ext.commands.Context = None,
        ...
        seconds = minutes * 60 if logger.level > logging.DEBUG else 3
        sentence = self.convert_seconds_to_emojis(seconds, **kwargs)
    ...
    async def on_every_second(self, remaining_seconds: int, message: discord.Message, **kwargs) -> None:
        try:
            await message.edit(content=self.convert_seconds_to_emojis(remaining_seconds, **kwargs))
        except discord.errors.NotFound:
            await message.channel.send(self.convert_seconds_to_emojis(remaining_seconds, **kwargs))
    ...
    @command()
    async def stop(self, ctx: Optional[discord.ext.commands.Context] = None, *, channel: discord.TextChannel = None)\
            -> None:
        channel = channel or ctx.channel
